(function() {
  ...
  function getFoodNamesOnly($log, $http, devEnvironment) {
    ...
    function getFoodNamesList(limit) {
      ...
      // Here we actually hit the API using our .env variables but not without concatenating a limit integer and our API key. We utilize an angular try-catch and depending on if successful or not we display error getFoodNamesFail() or return data returnFoodNamesList() to our malarkey.directive.js controller. For a list of all request parameters visit - https://ndb.nal.usda.gov/ndb/doc/apilist/API-LIST.md
